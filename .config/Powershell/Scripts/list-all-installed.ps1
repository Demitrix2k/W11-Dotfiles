function Get-AllInstalledPrograms {
    ...
    $allApps = ($desktopApps64 + $desktopApps32 + $storeApps) | Where-Object {$_.DisplayName -ne $null} | Sort-Object DisplayName | Select-Object DisplayName, DisplayVersion, Publisher, InstallDate

    # Output to console if -p flag is present, otherwise to a text file
    if ($p) {
        $allApps | Format-Table -AutoSize
    } else {
        $allAppsFormatted = $allApps | Format-Table -AutoSize | Out-String -Width 4096
        $allAppsFormatted | Out-File C:\Users\$env:USERNAME\Desktop\All_Installed_Programs.txt -Encoding UTF8
        Write-Host "Program list saved to C:\Users\$env:USERNAME\Desktop\All_Installed_Programs.txt"
    }
}

# Call the function with all passed parameters
Get-AllInstalledPrograms @args